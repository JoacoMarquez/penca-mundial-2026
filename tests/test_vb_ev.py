...
def _quote(odds=2.2, outcome="home", market="1x2", event_id="e1", hours_ahead=24.0,
    ...
    return OddsQuote(
        book="supermatch", sport=sport, league="liga", event_id=event_id,
...
def test_single_edge_exacto():
    ...
    out = find_singles([(q, fair, 2.05)], CFG, LearningState(), now_utc=NOW)
    assert len(out) == 1
...
def test_single_bajo_umbral_no_sugiere():
    ...
    fair = {"home": 0.50, "draw": 0.28, "away": 0.22}
    assert find_singles([(q, fair, 2.0)], CFG, LearningState(), now_utc=NOW) == []


def test_filtro_odds_range():
    q = _quote(odds=8.0)  # fuera de [1.4, 6.0] aunque el edge sea enorme
    fair = {"home": 0.30, "draw": 0.40, "away": 0.30}
    assert find_singles([(q, fair, 7.0)], CFG, LearningState(), now_utc=NOW) == []
...
def test_segmento_desactivado_no_sugiere():
    ...
    assert find_singles([(q, fair, 2.05)], CFG, state, now_utc=NOW) == []
...
def test_multiplicador_sube_umbral():
    ...
    assert find_singles([(q, fair, 2.0)], CFG, state, now_utc=NOW) == []
